...
const InvoiceAgingReport: React.FC<InvoiceAgingReportProps> = ({ factures, clients, onClose }) => {
  const [clientFilter, setClientFilter] = useState<string>('ALL');

  // Unique clients from outstanding invoices
  const uniqueClients = useMemo(() => {
    ...
    for (const f of outstanding) {
      ...
      if (!seen.has(clientId)) {
        seen.add(clientId);
        const nom =
          clients.find(c => c.id === clientId)?.nom_entreprise ||
          f.client_nom ||
          'Client inconnu';
        result.push({ id: clientId, nom });
      }
    }
  ...
  const buckets = useMemo(() => {
    const outstanding = factures.filter(
      f =>
        f.soldeRestant > 0 &&
        f.statut_paiement !== 'ANNULE' &&
        (clientFilter === 'ALL' || f.client_id === clientFilter)
    );

    const grouped: Record<string, FactureWithPayments[]> = {
      '0-30': [],
      '31-60': [],
      '61-90': [],
      '90+': [],
  ...
  const grandTotal = useMemo(
    () =>
      Object.values(buckets).reduce(
        (sum, invoices) => sum + invoices.reduce((s, f) => s + f.soldeRestant, 0),
        0
      ),
    [buckets]
  );

  const getClientName = (facture: FactureWithPayments): string =>
    clients.find(c => c.id === facture.client_id)?.nom_entreprise ||
    facture.client_nom ||
    'Client inconnu';

  const getDaysOverdue = (facture: FactureWithPayments): number =>
    Math.max(
      0,
      Math.floor(
        (Date.now() - new Date(facture.date_echeance || Date.now()).getTime()) / 86400000
      )
    );
            ...
            <>
              {(['0-30', '31-60', '61-90', '90+'] as const).map(bucketKey => {
                ...
                return (
                  <div
                    key={bucketKey}
                    className={`border rounded-lg overflow-hidden ${BUCKET_COLORS[bucketKey]}`}
                  >
                    {/* Bucket header */}
                    <div
                      className={`flex items-center justify-between px-4 py-2.5 ${BUCKET_HEADER_COLORS[bucketKey]}`}
                    >
                      <span className="font-semibold text-sm">
                        {BUCKET_LABELS[bucketKey]} — {invoices.length} facture(s)
                      </span>
                      {invoices.length > 0 && (
                        <span className="font-bold text-sm">
                          Total : {formatCurrency(bucketTotal, invoices[0]?.devise || 'USD')}
                        </span>
                      )}
                    </div>
                    ...
                    {invoices.length === 0 ? (
                      <div className="px-4 py-3 text-sm text-gray-400 italic">
                        Aucune facture dans cette tranche.
                      </div>
                    ) : (
                        ...
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {invoices.map(f => (
                              <tr
                                key={f.id}
                                className="border-t border-white border-opacity-60 hover:bg-white hover:bg-opacity-40 transition-colors"
                              >
                                ...
                                <td className="px-4 py-2 text-right text-gray-700">
                                  {formatCurrency(f.montant_total_du_client, f.devise)}
                                </td>
                                <td className="px-4 py-2 text-right text-gray-700">
                                  {formatCurrency(f.totalPaye, f.devise)}
                                </td>
                                <td className="px-4 py-2 text-right font-semibold text-gray-900">
                                  {formatCurrency(f.soldeRestant, f.devise)}
                                </td>
                            ...
                            <tr className="border-t-2 border-white bg-white bg-opacity-50 font-semibold">
                              <td colSpan={4} className="px-4 py-2 text-gray-700">
                                Sous-total {BUCKET_LABELS[bucketKey]} ({invoices.length} facture
                                {invoices.length > 1 ? 's' : ''})
                              </td>
                              <td className="px-4 py-2" />
                              <td className="px-4 py-2 text-right text-gray-900">
                                {formatCurrency(bucketTotal, invoices[0]?.devise || 'USD')}
                              </td>
                              <td className="px-4 py-2" />
                            </tr>
                          </tbody>
