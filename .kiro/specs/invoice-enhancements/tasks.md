...
- [ ] 2. Sequential invoice numbering (backend)
  - [ ] 2.1 Implement `getNextInvoiceNumber(dateEmission, db)` in electron.cjs
  ...
  - [ ]* 2.2 Write property test for FAC number format (Property 1)
  ...
  - [ ]* 2.3 Write property test for sequence strict increment (Property 2)
  ...
  - [ ]* 2.4 Write property test for deleted numbers not reused (Property 3)
  ...
  - [ ] 2.5 Wire `getNextInvoiceNumber` into `db-add-facture-gas` handler
...
- [ ] 4. Automatic payment status transitions (backend)
  - [ ] 4.1 Implement `updateFacturePaymentStatus(factureId, db)` in electron.cjs
  ...
  - [ ]* 4.2 Write property test for payment status engine (Property 4)
  ...
  - [ ]* 4.3 Write property test for protected statuses (Property 5)
  ...
  - [ ] 4.4 Wire `updateFacturePaymentStatus` into `db-add-paiement-gas` and `db-delete-paiement-gas` handlers
  ...
  - [ ] 4.5 Update `db-get-facture-paiements-summary` handler to include avoir amounts in `solde_restant`
...
- [ ] 6. Overdue status — EN_RETARD (frontend)
  - [ ] 6.1 Implement `isOverdue(facture)` pure function in InvoicesManagement.tsx
    - Returns `true` iff `date_echeance < today`, `soldeRestant > 0`, and status is not `PAYE_TOTAL` or `ANNULE`
    - _Requirements: 3.1, 3.2_

  - [ ]* 6.2 Write property test for `isOverdue` pure predicate (Property 6)
  ...
  - [ ]* 6.3 Write property test for EN_RETARD never persisted (Property 7)
  ...
  - [ ] 6.4 Update `getStatusConfig` to accept `overdue` flag and return EN_RETARD config
    - Return `{ label: 'En Retard', color: 'bg-red-100 text-red-800', icon: AlertCircle }` when `overdue = true`
    - _Requirements: 3.3_

  - [ ] 6.5 Wire EN_RETARD into invoice list rendering, status filter, and statistics card
...
- [ ] 7. Credit notes — Avoir (backend + frontend)
  - [ ] 7.1 Implement `getNextAvoirNumber(dateAvoir, db)` in electron.cjs
    - Use `avoir_sequences` table with year-only key
    - Return formatted `AV-{YYYY}-{NNN}` string
    - _Requirements: 4.1_

  - [ ]* 7.2 Write property test for AV number format (Property 8)
    - **Property 8: AV number format**
    - **Validates: Requirements 4.1**
    - Generate random ISO date strings; assert result matches `/^AV-\d{4}-\d{3}$/`

  - [ ] 7.3 Implement `db-create-avoir` IPC handler in electron.cjs
  ...
  - [ ]* 7.4 Write property test for avoir over-credit rejection (Property 9)
  ...
  - [ ] 7.5 Implement `db-get-avoirs-for-facture` and `db-get-avoirs-for-client` IPC handlers in electron.cjs
  ...
  - [ ] 7.6 Add `createAvoir`, `getAvoirsForFacture`, `getAvoirsForClient` to preload.cjs
    - Wire each to the corresponding `ipcRenderer.invoke` call
    - _Requirements: 4.3, 4.5, 4.8_

  - [ ] 7.7 Add `AvoirGAS`, `AgingBucket`, `StatementLine` TypeScript interfaces to `src/types/index.ts`
    - _Requirements: 4.2_

  - [ ] 7.8 Create `CreditNoteForm.tsx` modal component
  ...
  - [ ] 7.9 Create `CreditNotePrintTemplate.tsx` A4 print component
  ...
  - [ ] 7.10 Wire credit notes into `InvoiceDetailModal`
...
- [ ] 9. Invoice aging report (frontend)
  - [ ] 9.1 Implement `getBucket(facture)` pure function
    - Returns `'0-30' | '31-60' | '61-90' | '90+'` based on days since `date_echeance`
    - No `date_echeance` → `'0-30'`
    - _Requirements: 5.2, 5.3_

  - [ ]* 9.2 Write property test for aging bucket assignment (Property 10)
  ...
  - [ ]* 9.3 Write property test for aging bucket totals (Property 11)
  ...
  - [ ]* 9.4 Write property test for aging client filter completeness (Property 12)
  ...
  - [ ] 9.5 Create `InvoiceAgingReport.tsx` modal component
  ...
  - [ ] 9.6 Add "Rapport d'Ancienneté" button to `InvoicesManagement` and wire `InvoiceAgingReport` modal
    - _Requirements: 5.1_

- [ ] 10. Client statement (frontend)
  - [ ] 10.1 Create `ClientStatement.tsx` modal component
  ...
  - [ ]* 10.2 Write property test for closing balance invariant (Property 13)
  ...
  - [ ]* 10.3 Write property test for statement transaction completeness (Property 14)
  ...
  - [ ] 10.4 Add "Relevé de Compte" button per client in `InvoicesManagement` and wire `ClientStatement` modal
    - _Requirements: 6.1_
